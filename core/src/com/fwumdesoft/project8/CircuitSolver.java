...
//		System.out.println(system.solve());
		...
		v.voltageDif = 10;
		r.resistance = 5;
		CircuitComponent[][] circuit = new CircuitComponent[][] {{n, n, n, n, n, n, n}, {w, w, w, w, w, w, n}, {v, n, n, w, n, w, n},
...
//		System.out.println(buildEquation(circuit, junctions, branches, branches.get(0).startDirection, branches.get(0).start, branches.get(0), branches.get(0), new ArrayList<Branch>()));
	...
	/**
	 * Takes in a 2D array representing a PROPER circuit, and sets each component's
	 * current value to its proper amount
	 * @param circuit Represents the 2D of a circuit to be solved
	 */
	 ...
	 * @param circuit The 2D array representing circuit
	 * @param branch The branch currently being filled
	 * @param loc The start location of branch iteration
	...
	/**
	 * Using Kirchoff's Loop Rule, will build an equation recursively from the startBranch
	 * @param circuit The 2D array representing the circuit
	 * @param junctions The list of junctions in the circuit
	 * @param branches The list of branches in the circuit
	 * @param loc The start location of this call
	 * @param prev The "previous" location to loc (used to move in the proper direction)
	 * @param currentBranch The branch this call is iterating on
	 * @param startBranch The initial branch, used as the base case
	 * @param checkedBranches The branches already being checked, used to make sure the path doesn't loop where it shouldn't
	 * @return An ArrayList of Terms for an equation, not necessarily in order according to "branches"
	 */
	private static ArrayList<Term> buildEquation(CircuitComponent[][] circuit, ArrayList<Vector2> junctions, ArrayList<Branch> branches, Vector2 prev,
			Vector2 loc, Branch currentBranch, Branch startBranch, ArrayList<Branch> checkedBranches) {
		...
		if(startBranch.start.equals(loc) || startBranch.start.equals(loc) || startBranch.start.equals(loc) || startBranch.start.equals(loc))
			return workingSet;
		...
		for(Branch branch : branches) {
			if(!checkedBranches.contains(branch) && (branch.start.equals(loc) || branch.end.equals(loc))) {
				...
				result = buildEquation(circuit, junctions, branches, loc,
						new Vector2(branch.start.equals(loc) ? branch.startDirection : branch.endDirection), branch, startBranch, checkedBranches);
				if(result != null)
					break;
			}
		}
		...
		if(result == null) {
			...
			return null;
		} else {//Otherwise, succeed by trace-back
	 ...
	 * @param circuit The 2D array representing the circuit
	 * @param junctions The list of Vector2s to be filled with junctions
	 */
	...
	/**
	 * Fills branches with references to branches in the circuit, with branch.start on the start junction and branch.end just before the
	 * end junction
	 * @param circuit The 2D array representing the circuit
	 * @param junctions The list of junctions in the circuit
	 * @param branches The list to be filled with branches
	 */
	...
	private static void buildBranch(CircuitComponent[][] circuit, ArrayList<Vector2> junctions, ArrayList<Branch> branches, Vector2 prev,
			Vector2 loc) {
		...
		while(cont)
			if(junctions.contains(loc))
				cont = false;
			else if(loc.x + 1 < circuit.length && !new Vector2(loc.x + 1, loc.y).equals(prev) && circuit[(int)loc.x + 1][(int)loc.y] != null) {
				prev.set(loc);
				loc.x++;
	...
	private static class SystemOfEquations {
		...
		/**
		 * Solves the system
		 * @return An ArrayList of Doubles which are the currents through each branch
		 */
		public ArrayList<Double> solve() {
			int numVars = equations.get(0).terms.size();
			//By the end of this loop, the first "numVars" equations will be in Reduced Row Echeleon Form
			for(int row = 0; row < numVars; row++) {
				...
				//Zero everything above and below the 1, for reduced row echeleon
				for(int e = 0; e < equations.size(); e++)
					if(e == row)
						continue;
					else if(equations.get(e).terms.get(row).coefficient != 0)
						equations.set(e, equations.get(row).scalar(-equations.get(e).terms.get(row).coefficient).add(equations.get(e)));
			}
	...
	private static class Equation {
		...
		/**
		 * Rearranges the terms according to branches and adds terms of 0 for missing branches
		 ...
		 * @param scale The value to multiply the equation by
		...
		/**
		 * Adds two equations together and returns the result, combining like terms
		 * @param equation The other equation to be added
